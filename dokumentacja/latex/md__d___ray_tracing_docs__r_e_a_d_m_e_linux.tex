By default SDL will only link against glibc, the rest of the features will be enabled dynamically at runtime depending on the available features on the target system. So, for example if you built SDL with Xinerama support and the target system does not have the Xinerama libraries installed, it will be disabled at runtime, and you won\textquotesingle{}t get a missing library error, at least with the default configuration parameters.\hypertarget{md__d___ray_tracing_docs__r_e_a_d_m_e_linux_autotoc_md59}{}\doxysection{Build Dependencies}\label{md__d___ray_tracing_docs__r_e_a_d_m_e_linux_autotoc_md59}
...
\end{DoxyItemize}\hypertarget{md__d___ray_tracing_docs__r_e_a_d_m_e_linux_autotoc_md60}{}\doxysection{Joystick does not work}\label{md__d___ray_tracing_docs__r_e_a_d_m_e_linux_autotoc_md60}
