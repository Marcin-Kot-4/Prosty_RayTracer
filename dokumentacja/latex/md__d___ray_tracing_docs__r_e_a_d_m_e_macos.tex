...
From the developer\textquotesingle{}s point of view, mac\+OS is a sort of hybrid Mac and Unix system, and you have the option of using either traditional command line tools or Apple\textquotesingle{}s IDE Xcode.\hypertarget{md__d___ray_tracing_docs__r_e_a_d_m_e_macos_autotoc_md62}{}\doxysection{Command Line Build}\label{md__d___ray_tracing_docs__r_e_a_d_m_e_macos_autotoc_md62}
...
To use the library once it\textquotesingle{}s built, you essential have two possibilities\+: use the traditional autoconf/automake/make method, or use Xcode.\hypertarget{md__d___ray_tracing_docs__r_e_a_d_m_e_macos_autotoc_md63}{}\doxysection{Caveats for using SDL with Mac OS X}\label{md__d___ray_tracing_docs__r_e_a_d_m_e_macos_autotoc_md63}
...
\end{DoxyCode}
\hypertarget{md__d___ray_tracing_docs__r_e_a_d_m_e_macos_autotoc_md64}{}\doxysection{Using the Simple Direct\+Media Layer with a traditional Makefile}\label{md__d___ray_tracing_docs__r_e_a_d_m_e_macos_autotoc_md64}
...
As a final remark, let me add that I use some of the techniques (and some variations of them) in \href{https://github.com/exult/exult}{\texttt{ Exult}} and \href{https://github.com/scummvm/scummvm}{\texttt{ Scumm\+VM}}; both are available in source on the net, so feel free to take a peek at them for inspiration!\hypertarget{md__d___ray_tracing_docs__r_e_a_d_m_e_macos_autotoc_md65}{}\doxysection{Using the Simple Direct\+Media Layer with Xcode}\label{md__d___ray_tracing_docs__r_e_a_d_m_e_macos_autotoc_md65}
These instructions are for using Apple\textquotesingle{}s Xcode IDE to build SDL applications.\hypertarget{md__d___ray_tracing_docs__r_e_a_d_m_e_macos_autotoc_md66}{}\doxysubsection{First steps}\label{md__d___ray_tracing_docs__r_e_a_d_m_e_macos_autotoc_md66}
...
This will create a new folder called Xcode, which you can browse normally from the Finder.\hypertarget{md__d___ray_tracing_docs__r_e_a_d_m_e_macos_autotoc_md67}{}\doxysubsection{Building the Framework}\label{md__d___ray_tracing_docs__r_e_a_d_m_e_macos_autotoc_md67}
...
\end{DoxyItemize}\hypertarget{md__d___ray_tracing_docs__r_e_a_d_m_e_macos_autotoc_md68}{}\doxysubsection{Build Options}\label{md__d___ray_tracing_docs__r_e_a_d_m_e_macos_autotoc_md68}
There are two \char`\"{}\+Build Styles\char`\"{} (See the \char`\"{}\+Targets\char`\"{} tab) for SDL. \char`\"{}\+Deployment\char`\"{} should be used if you aren\textquotesingle{}t tweaking the SDL library. \char`\"{}\+Development\char`\"{} should be used to debug SDL apps or the library itself.\hypertarget{md__d___ray_tracing_docs__r_e_a_d_m_e_macos_autotoc_md69}{}\doxysubsection{Building the Testers}\label{md__d___ray_tracing_docs__r_e_a_d_m_e_macos_autotoc_md69}
Open the SDLTest project and build away!\hypertarget{md__d___ray_tracing_docs__r_e_a_d_m_e_macos_autotoc_md70}{}\doxysubsection{Using the Project Stationary}\label{md__d___ray_tracing_docs__r_e_a_d_m_e_macos_autotoc_md70}
Copy the stationary to the indicated folders to access it from the \char`\"{}\+New Project\char`\"{} and \char`\"{}\+Add target\char`\"{} menus. What could be easier?\hypertarget{md__d___ray_tracing_docs__r_e_a_d_m_e_macos_autotoc_md71}{}\doxysubsection{Setting up a new project by hand}\label{md__d___ray_tracing_docs__r_e_a_d_m_e_macos_autotoc_md71}
...
\begin{DoxyItemize}
\item Click \char`\"{}\+File\char`\"{} -\/\texorpdfstring{$>$}{>} \char`\"{}\+New\char`\"{} -\/\texorpdfstring{$>$}{>} \char`\"{}\+Project... $\ast$ Choose \char`\"{}mac\+OS\char`\"{} and then \char`\"{}App\char`\"{} from the \char`\"{}Application\char`\"{} section. $\ast$ Fill out the options in the next window. User interface is \char`\"{}XIB\char`\"{} and   Language is \char`\"{}Objective-\/C\char`\"{}. $\ast$ Remove \char`\"{}main.\+m\char`\"{} from your project $\ast$ Remove \char`\"{}Main\+Menu.\+xib\char`\"{} from your project $\ast$ Remove \char`\"{}App\+Delegates.$\ast$\char`\"{} from your project $\ast$ Add \char`\"{}\$(HOME)/\+Library/\+Frameworks/\+SDL.framework/\+Headers\char`\"{} to include path $\ast$ Add \char`\"{}\$(HOME)/\+Library/\+Frameworks\char`\"{} to the frameworks search path $\ast$ Add \char`\"{}-\/framework SDL -\/framework Foundation -\/framework App\+Kit\char`\"{} to \char`\"{}OTHER\+\_\+\+LDFLAGS\char`\"{} $\ast$ Add your files $\ast$ Clean and build  @subsection autotoc\+\_\+md72 Building from command line  \+Use $<$tt$>$xcode-\/build$<$/tt$>$ in the same directory as your .\+pbxproj file  @subsection autotoc\+\_\+md73 Running your app  \+You can send command line args to your app by either invoking it from the command line (in $\ast$.\+app/\+Contents/\+Mac\+OS) or by entering them in the \+Executables\char`\"{} panel of the target settings.
\end{DoxyItemize}\hypertarget{md__d___ray_tracing_docs__r_e_a_d_m_e_macos_autotoc_md74}{}\doxysection{Implementation Notes}\label{md__d___ray_tracing_docs__r_e_a_d_m_e_macos_autotoc_md74}
Some things that may be of interest about how it all works...\hypertarget{md__d___ray_tracing_docs__r_e_a_d_m_e_macos_autotoc_md75}{}\doxysubsection{Working directory}\label{md__d___ray_tracing_docs__r_e_a_d_m_e_macos_autotoc_md75}
In SDL 1.\+2, the working directory of your SDL app is by default set to its parent, but this is no longer the case in SDL 2.\+0. SDL2 does change the working directory, which means it\textquotesingle{}ll be whatever the command line prompt that launched the program was using, or if launched by double-\/clicking in the finger, it will be \char`\"{}/\char`\"{}, the {\itshape root of the filesystem}. Plan accordingly! You can use \mbox{\hyperlink{_s_d_l__filesystem_8h_af4bb772b30931388d2efcbfe30bf99e9}{SDL\+\_\+\+Get\+Base\+Path()}} to find where the program is running from and chdir() there directly.\hypertarget{md__d___ray_tracing_docs__r_e_a_d_m_e_macos_autotoc_md76}{}\doxysubsection{You have a Cocoa App!}\label{md__d___ray_tracing_docs__r_e_a_d_m_e_macos_autotoc_md76}
Your SDL app is essentially a Cocoa application. When your app starts up and the libraries finish loading, a Cocoa procedure is called, which sets up the working directory and calls your \mbox{\hyperlink{_c_make_c_compiler_id_8c_a0ddf1224851353fc92bfbff6f499fa97}{main()}} method. You are free to modify your Cocoa app with generally no consequence to SDL. You cannot, however, easily change the SDL window itself. Functionality may be added in the future to help this.\hypertarget{md__d___ray_tracing_docs__r_e_a_d_m_e_macos_autotoc_md77}{}\doxysection{Bug reports}\label{md__d___ray_tracing_docs__r_e_a_d_m_e_macos_autotoc_md77}
